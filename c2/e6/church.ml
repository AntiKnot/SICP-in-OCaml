...
let ch_zero : church_num = 
    (* let f = fun f x -> x  will error unbound f*)
    let f = fun _ x -> x
    in {f}
...
let () =
printf "zero succ %N\n" (int_of_church (ch_zero|> ch_succ));;
printf "zero succ %N\n" (int_of_church (ch_succ ch_zero));;
printf "church four %N\n" (int_of_church ch_four);;
printf "church ch81 %N\n" (int_of_church ch_81);;

(* 怎么理解邱奇计数？ - Ludwig Snow的回答 - 知乎 https://www.zhihu.com/question/39930042/answer/84865437 *)
(* Problem from codewars https://www.codewars.com/kata/525f3eda17c7cd9f9e000b39/python *)
(* https://rosettacode.org/wiki/Church_numerals#OCaml *)